export const API = {
  LOGIN: '/api/v1/user/login',
  LOGOUT: '/api/v1/user/logout',
  PROFILE: '/api/v1/user/profile',
  USERS: '/api/v1/users',
  USER_REGISTER: '/api/v1/user/register',
  USER_UPDATE: '/api/v1/user/update',
  USER_INDEX: '/api/v1/user/index',
  PROFILES: '/api/v1/profiles',
  PROFILE_INDEX: '/api/v1/profile/index',
  PROFILE_CREATE: '/api/v1/profile/create',
  PROFILE_UPDATE: '/api/v1/profile/update',
  PROFILE_BY_TRANSACTION_TYPE: '/api/v1/profile/get-profile-transaction-type',
  UPLINK_PROFILES_ALL: '/api/v1/profile/get-all-uplink',
  TRANSACTION_CREATE: '/api/v1/transaction/create',
  TRANSACTION_INDEX: '/api/v1/transaction/index',
  DASHBOARD_INDEX: '/api/v1/dashboard/index',
  TRANSACTIONS: '/api/v1/transactions',
  TRANSACTION_TYPES: '/api/v1/transaction-type/all',
  TRANSACTION_TYPE_INDEX: '/api/v1/transaction-type/index',
  TRANSACTION_TYPE_CREATE: '/api/v1/transaction-type/create',
  TRANSACTION_TYPE_UPDATE: '/api/v1/transaction-type/update',
  TRANSACTION_TYPE_DELETE: '/api/v1/transaction-type/delete',
  LEDGER: '/api/v1/ledger',
  LEDGER_INDEX: '/api/v1/ledger/index',
  SETTLEMENTS: '/api/v1/settlements',
  REPORTS: '/api/v1/reports',
  AUDIT: '/api/v1/audit',
} as const;
